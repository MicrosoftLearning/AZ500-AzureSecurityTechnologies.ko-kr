...
1. **Log Analytics 작업 영역** 블레이드에서 **+ 만들기**를 클릭합니다.
...
1. **데이터 원본 연결** 섹션의 Log Analytics 작업 영역 블레이드에서 **Azure VM(가상 머신)** 항목을 클릭합니다.
...
1. **Windows 이벤트 로그**가 선택되어 있는지 확인하고 **+ Windows 이벤트 로그 추가**를 클릭한 다음 이벤트 로그 유형 목록에서 **시스템**을 선택합니다.
