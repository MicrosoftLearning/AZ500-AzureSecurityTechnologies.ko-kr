...
1. **액세스 정책 추가** 블레이드에서 다음 설정을 지정합니다. 다른 모든 설정은 기본값으로 유지합니다. 
    ...
    |주요 권한|**모두 선택**을 클릭하면 **16개의 선택된** 권한이 나타납니다(**회전 정책 작업**에 대한 권한을 **선택 취소**해야 함). |
...
1. Azure Portal 내 **medical** SQL 데이터베이스의 블레이드로 이동하여 **필수** 섹션에서 **서버 이름** (클립보드에 복사)을 확인합니다. 그런 다음 도구 모음에서 **서버 방화벽 설정**을 클릭합니다.  
...
**참고:** 암호화를 수행하는 동안 **회전(Microsoft.SQLServer.Management.ServiceManagement)** 과 관련하여 **적용 대상에서 예외가 발생함**이라는 오류가 표시되는 경우 **회전 정책 작업**의 **주요 권한** 값이 **선택 취소**되어 있는지 확인합니다. 선택 취소되어 있지 않으면 Azure Portal에서 **Key Vault** >> **액세스 정책** >> **주요 권한**으로 이동하여 **회전 정책 작업** 아래에 있는 모든 값을 선택 취소합니다. 
...
1. **개체 탐색기** 창에서 **의료 데이터베이스**를 마우스 오른쪽 단추로 클릭하고 마우스 오른쪽 클릭 메뉴에서 **새 쿼리**를 클릭합니다.
