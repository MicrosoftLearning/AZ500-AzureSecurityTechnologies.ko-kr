...
9. Active Directory 관리Istrative Center 콘솔의 세부 정보 창에 콘텐츠가 표시되도록 새로 만든 **ToSync** 조직 구성 단위를 두 번 클릭합니다. 
...
3. Azure Portal**의 AdatumSync \| 개요** 블레이드의 관리** 아래 왼쪽 탐색 패널에서 **Azure AD 커넥트** 클릭합니다**.

4. **AAD 커넥트 \| 시작** 블레이드**의 왼쪽 탐색 패널에서 커넥트 동기화**를 클릭한 다음 Azure AD 커넥트** 다운로드 링크를 클릭합니다**. Azure AD 커넥트** 다운로드 페이지로 **리디렉션됩니다.

5. **Azure AD 커넥트** 다운로드 페이지에서 다운로드**를 클릭합니다**.
...
5. adVM에 대한 **원격 데스크톱 세션 내에서 Active Directory 관리영구 센터로** 전환**하고 ToSync** OU의 개체 **목록에서 aduser1** 항목을 선택하고 **작업 **** 창**의 aduser1** 섹션에서 속성을** 선택합니다**.**
