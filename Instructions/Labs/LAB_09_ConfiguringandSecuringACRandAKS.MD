...
1. **Kubernetes Service** 블레이드에서 **+ 만들기**를 클릭하고 드롭다운 메뉴에서 **+ Kubernetes 클러스터 만들기**를 클릭합니다.

1. **Create 클러스터 만들기** 블레이드의 **기본** 탭에서 **모든 사전 설정 구성 보기**를 클릭하고 **개발/테스트($)** 를 선택한 다음 **적용**을 클릭합니다. 이제 다음 설정을 지정합니다(다른 설정은 기본값을 그대로 사용).
